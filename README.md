...
The public blog loads articles from `posts.json`, so posts are stable across laptop, mobile, and other browsers once that file is committed to GitHub. After changing articles in Studio, click **Download posts.json**, replace the repository's `posts.json` with that download, commit it, and push to GitHub Pages. Because GitHub Pages is static hosting, Studio cannot write directly back to GitHub by itself.
